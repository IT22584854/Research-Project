    ...
    *,
    ...
    if existing and not existing_pdf_is_healthy(existing):
        if (ARCHIVE_DIR in existing.parents) or (existing.parent == ARCHIVE_DIR):
            print(f" SKIP_BAD_ARCHIVE_EXISTS: {filename}")
            await append_pdf_index(
                {
                    "ts": utc_ts(),
                    "site": job.site_prefix,
        ...
        if not repair_bad:
            print(f" SKIP_BAD_EXISTS (repair disabled): {filename}")
            await append_pdf_index(
                {
                    "ts": utc_ts(),
                    "site": job.site_prefix,
    ...
    if out_path.exists():
        ...
        if repair_bad:
            ...
            print(f" SKIP_EXISTING_UNSAFE: {filename}")
            await append_pdf_index(
                {
                    "ts": utc_ts(),
                    "site": job.site_prefix,
