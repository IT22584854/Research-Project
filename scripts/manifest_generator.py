...
def infer_raw_path(clean_rel: str) -> str:
    # still mirrors 0_raw structure (because scrubber mirrors it)
    ...
    with SKIP_LOG.open("w", newline="", encoding="utf-8") as flog:
        ...
        with OUT_PATH.open("w", encoding="utf-8") as out:
            for p in files:
                ...
                # ✅ compute stable source_key early (after normalization)
                ...
                # ✅ doc_id stability:
                # Prefer stable identity from source_key; fallback to content hash only if source identity missing.
                ...
                record = {
                    ...
                    # PDF provenance
                    "source_pdf": source_pdf,                 # filename (hashed name)
                    "source_pdf_url": source_pdf_url,         # original URL (best)
