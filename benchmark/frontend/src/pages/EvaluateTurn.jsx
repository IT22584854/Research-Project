...
import { evalHighRiskTurn, evalGroundTruthTurn } from "../api";

export default function EvaluateTurn() {
  ...
  const run = async (type) => {
    ...
    try {
      ...
      setResult({ error: "Failed to evaluate turn. Please check the Turn ID and try again." });
    } finally {
      setLoading(false);
    }
  };
  ...
  const getResultStyle = () => {
    if (result?.error) {
      return { ...styles.resultContent, ...styles.errorState };
    }
    if (result && !result.error) {
      return { ...styles.resultContent, ...styles.successState };
    }
    return styles.resultContent;
  };

  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h3 style={styles.title}>Evaluate Single Turn</h3>
      ...
      <div style={styles.card}>
        <div style={styles.inputSection}>
          <div style={styles.inputWrapper}>
            <label style={styles.label}>
              ...
              Turn ID
              <span style={styles.labelHint}>(e.g., turn_123 or 123)</span>
            </label>
            <input
              style={styles.input}
              placeholder="Enter turn ID to evaluate..."
              value={id}
        ...
        <div style={styles.resultSection}>
          <div style={styles.resultTitle}>
            ...
            {result && !loading && (
              <span style={{ fontSize: '12px', fontWeight: '400', color: '#8e8e8e', marginLeft: 'auto' }}>
                Turn ID: {id}
              </span>
            )}
          </div>
          ...
          {loading ? (
            ...
            <div style={styles.emptyState}>
              ...
              <p style={{ margin: '8px 0 0 0', fontSize: '14px' }}>
                Enter a Turn ID and click a button to see evaluation results
              </p>
              <p style={{ margin: '8px 0 0 0', fontSize: '12px', color: '#c0c0c0' }}>
                Press Enter to quickly evaluate with High Risk
              </p>
            </div>
          )}
        </div>
